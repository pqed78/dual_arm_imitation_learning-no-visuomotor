# Dual Arm IL 프로젝트 전체 프롬프트

이 파일은 Antigravity AI 또는 다른 LLM을 사용하여 현재 프로젝트를 100% 동일하게 복원하기 위해 설계된 마스터 프롬프트입니다. 아래의 전체 텍스트를 복사하여 에이전트에게 전달하세요.

---

**역할 및 목표:**
너는 Isaac Lab과 PyTorch를 기반으로 로봇 팔 제어 및 모방 학습(Imitation Learning) 환경을 구축하는 전문가야.
기존의 강화학습(RL) 환경 코드를 건드리지 않고, 독립적인 모방 학습 전용 프로젝트 폴더(`dual_arm_il`)를 생성해서 완전한 IL 파이프라인을 구성해 줘.


**[에이전트 필수 행동 수칙 (CRITICAL)]**
1. **설명만 하지 말고 파일 생성하기:** 아래 제공되는 모든 파일 경로와 소스 코드를 바탕으로 실제 디렉토리에 빠짐없이 파일을 생성해 줘.
2. **코드 생략 절대 금지:** 아래 텍스트의 파일 내용을 100% 동일하게 작성해 줘. `pass`, `TODO`, `...` 등을 사용해서 코드를 축약하면 절대 안 돼.
3. **폴더 구조 유지:** 제공된 파일 경로에 맞게 필요한 폴더를 생성하고 정확히 위치시켜 줘.
