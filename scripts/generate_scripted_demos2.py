...
def main():
    ...
    while simulation_app.is_running() and collected_count < target_count:
        ...
        for step in range(args_cli.max_steps_per_ep):
            ...
            if 5 <= phase <= 11 and obj_pos[:, 2].min().item() < 0.05:
            ...
            right_gripper_width = torch.sum(robot.data.joint_pos[:, right_gripper_joint_ids], dim=-1)
            left_gripper_width = torch.sum(robot.data.joint_pos[:, left_gripper_joint_ids], dim=-1)
            ...
            if phase == PHASE_INIT:
                ...
                step_size = torch.clamp(dist, max=0.012).unsqueeze(-1)
                delta_r_pos = (err / (dist.unsqueeze(-1) + 1e-6)) * step_size
                ...
                reached = (dist_xy < 0.010) & (dist_z < 0.005)
                if reached.all().item() or phase_timer > 160:
                    if reached.all().item():
                ...
                right_gripper_cmd = -1.0
                if phase_timer > 60:
                    if (right_gripper_width > 0.025).all().item():
                        print(f"  [Right Grasp Verified!] Step {step:03d} | Width: {right_gripper_width.mean().item()*1000:.1f}mm")
                        ...
                        print(f"  [Right Grasp Missed!] Step {step:03d} | Width: {right_gripper_width.mean().item()*1000:.1f}mm < 25mm. Retrying descend...")
                        right_gripper_cmd = 1.0
                        if phase_timer > 75:
                            phase = PHASE_RIGHT_DESCEND
                            phase_timer = 0
                ...
                delta_r_pos = (err / (dist.unsqueeze(-1) + 1e-6)) * step_size
                delta_r_rot = compute_desired_grasp_rot(wrist_quat_r, latched_cube_z, gain=0.2, max_rot_step=0.020)
                if dist.max().item() < 0.025 or phase_timer > 70:
                ...
                if (dist.max().item() < 0.03 and rot_aligned.all().item()) or phase_timer > 400:
                ...
                dist_xy = torch.norm(left_grasp_tgt[:, :2] - tcp_pos_l[:, :2], dim=-1)
                ...
                step_size = torch.clamp(dist, max=0.012).unsqueeze(-1)
                delta_l_pos = (err / (dist.unsqueeze(-1) + 1e-6)) * step_size
                ...
                reached_l = (dist_xy < 0.025) & (dist_z < 0.015)
                if reached_l.all().item() or phase_timer > 160:
                    if reached_l.all().item():
                ...
                if dist.max().item() < 0.03 or phase_timer > 60:
                ...
                step_size = torch.clamp(torch.norm(err, dim=-1), max=0.012).unsqueeze(-1)
                delta_r_pos = (err / (torch.norm(err, dim=-1).unsqueeze(-1) + 1e-6)) * step_size
                ...
                if dist.max().item() < 0.03 or phase_timer > 70:
                ...
                left_orient_target = -torch.sign(target_cube_z[:, 1:2] + 1e-6) * target_cube_z
                delta_l_rot = compute_desired_grasp_rot(wrist_quat_l, left_orient_target, gain=0.2, max_rot_step=0.020, directed=True)
                if (dist.max().item() < 0.015 or dist_z.max().item() < 0.006) or phase_timer > 60:
                ...
                step_size = torch.clamp(dist, max=0.012).unsqueeze(-1)
                delta_l_pos = (err / (dist.unsqueeze(-1) + 1e-6)) * step_size
                if dist.max().item() < 0.03 or phase_timer > 40:
                    phase = PHASE_SUCCESS
                    phase_timer = 0
